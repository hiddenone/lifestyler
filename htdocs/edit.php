...
function getCalendarByRange($id){
    error_log("Debug edit.php/GetCalendarByRange!!!!!");
  ...
  try{
    ...
    $handle = pg_query($sql);
    //echo $sql;
    $row = pg_fetch_object($handle);

	}catch(Exception $e){
        error_log('error in edit.php fetchhing id');
  }
  return $row;
  
} 
error_log('SSSSS DEBUG edit.php');
if($_GET["id"]){
  $event = getCalendarByRange($_GET["id"]);
}else{
    error_log('No Id for edit.php, this could be bad maybe?');
}
?>
